...
class Send2GECursor(Enum):
    """
    Enum representing available cursors for the Send2GE plugin.

    :cvar IDENTIFY: Cursor for the "Identify" tool.
    """

    IDENTIFY = _CursorMetadata("icons/identification.svg", 1, 1)


def create_cursor(cursor_metadata: Send2GECursor) -> QCursor:
    """
    Generate a QCursor object based on the provided Send2GEtCursor.

    This function creates a cursor using the icon and active point
    specified in the `_CursorMetadata` of the given `Send2GEtCursor`.
    ...
    icon = QIcon(str(Path(__file__).parent / cursor_metadata.value.icon))
